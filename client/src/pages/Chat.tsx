...
function Chat() {
  ...
  const [sessionLoad, setSessionLoad] = useState<boolean>(false);
  useEffect(() => {
    async function getSession(id: number) {
      ...
      if (!data?.document.isValid) {
        setAlert(
          "The document does not appear to be a research paper. Results may be unreliable.",
        );
      }
    }
    getSession(id);
        ...
        <div className="bg-sidebar flex-1 flex flex-col h-full pt-4 gap-4 overflow-y-auto messages-container px-2">
          {sessionLoad ? (
            <div className="flex flex-col w-full gap-4">
              <div className="flex items-center gap-4 self-end flex-row-reverse ">
                <Skeleton className="h-12 w-12 rounded-full bg-muted" />
                <div className="space-y-2">
                  <Skeleton className="h-15 w-[300px] bg-muted" />
                </div>
              </div>
              ...
              return (
                ...
                >
                  ...
                  <div
                    key={msg.id}
                    className={` ${
                      msg.role === "user" ? "bg-iconBg" : "bg-muted"
                    } p-4 rounded-md`}
                  >
